#!/usr/bin/env bash
# Kairos Maps — SOUTH_ASIAN editorial copy smoke (F2.7b)
# Content-only gate: packs exist · NOT registered F2.7c · no overlap · banlists · rotation · P03/P06/P10
set -euo pipefail
...
echo ""
echo "══════════════════════════════════════════════════════════"
echo " KAIROS MAPS — SOUTH_ASIAN editorial smoke (F2.7b)"
...
const OVERLAP_FAMILIES = [
  ...
  'LATAM', 'WESTERN_EUROPE', 'SOUTHEAST_ASIAN', 'GLOBAL_NEUTRAL'
];
...
const STEREO_BAN = [
  'espiritualidad', 'iluminación', 'templo', 'gurú', 'guru', 'karma', 'casta', 'curry',
  'taj mahal', 'exotismo', 'exotico', 'viaje interior', 'zen', 'lotus', 'monje'
];
...
assert(
  'SOUTH_ASIAN registered in resolver (F2.7c)',
...
assert(
  'resolveRegionalPack(SOUTH_ASIAN) 14/14 explicit',
...
assert('14/14 SOUTH_ASIAN packs exist', packMissing.length === 0, packMissing.join(' · ') || '14/14');
assert('14/14 packs non-empty', packEmpty.length === 0, packEmpty.join(' · ') || 'ok');

const saStrings = [];
PACK_MAPS.forEach(function (entry) {
  collectStrings(entry.map && entry.map[FAMILY], saStrings);
});

const placeholderHits = [];
saStrings.forEach(function (s) {
...
assert('rotation HUMAN_THEME ≥3/goal (WE parity)', countGoal(sa.theme, 3) && countGoal(we.theme, 3),
  JSON.stringify({ sa: sa.theme.amor.length, we: we.theme.amor.length }));
assert('rotation SUMMARY_FRAME ≥2/goal', countGoal(sa.frames, 2) && countGoal(we.frames, 2), 'ok');
assert('rotation OBSERVE_TAIL ≥2/goal', countGoal(sa.observe, 2) && countGoal(we.observe, 2), 'ok');
assert('rotation GOAL_PADS ≥8/goal', countGoal(sa.pads, 8) && countGoal(we.pads, 8), 'ok');
assert('rotation SPINE_FAVORECE ≥6', sa.favorece.length >= 6 && we.favorece.length >= 6,
  'sa=' + sa.favorece.length + ' we=' + we.favorece.length);
assert('rotation TOPUP ≥10', sa.topup.length >= 10 && we.topup.length >= 10,
  'sa=' + sa.topup.length + ' we=' + we.topup.length);
assert('rotation HUMAN_SCENE ≥3/goal', countGoal(sa.scene, 3) && countGoal(we.scene, 3), 'ok');
assert('rotation OBSERVE_ENTERO ≥2/goal', countGoal(sa.entero, 2) && countGoal(we.entero, 2), 'ok');
assert('rotation MICRO ≥2/goal', countGoal(sa.micro, 2) && countGoal(we.micro, 2), 'ok');
assert('rotation REGIONAL_PADS ≥8', sa.regionalPads.length >= 8 && we.regionalPads.length >= 8,
  'sa=' + sa.regionalPads.length + ' we=' + we.regionalPads.length);
...
assert('string count ~WE parity (≥180)', saStrings.length >= 180,
  'sa=' + saStrings.length + ' we=' + weStrings.length);
