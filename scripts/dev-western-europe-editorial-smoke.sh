#!/usr/bin/env bash
# Kairos Maps — WESTERN_EUROPE editorial copy smoke (F2.5b)
# Content-only gate: packs exist · no overlap · no banlist · no placeholders
set -euo pipefail
...
echo ""
echo "══════════════════════════════════════════════════════════"
echo " KAIROS MAPS — WESTERN_EUROPE editorial smoke (F2.5b)"
echo " Scope: 14 packs · anti-overlap · banlist · rotation · P03/P06/P10"
echo " Note: WESTERN_EUROPE registered in resolver (F2.5c runtime)"
echo "══════════════════════════════════════════════════════════"
echo ""
...
const OVERLAP_FAMILIES = ['IBERIAN', 'MEDITERRANEAN', 'ANGLO', 'LATAM', 'GLOBAL_NEUTRAL'];

const IBERIAN_BAN = ['plaza', 'sobremesa', 'barrio', 'compañía cotidiana'];
const MED_BAN = ['paseo', 'acera', 'vitrina urbana'];
...
assert(
  'WESTERN_EUROPE registered in resolver (F2.5c)',
...
assert(
  'resolveRegionalPack(WESTERN_EUROPE) 14/14 explicit',
...
assert('14/14 WESTERN_EUROPE packs exist', packMissing.length === 0, packMissing.join(' · ') || '14/14');
assert('14/14 packs non-empty', packEmpty.length === 0, packEmpty.join(' · ') || 'ok');

const weStrings = [];
PACK_MAPS.forEach(function (entry) {
  collectStrings(entry.map && entry.map[FAMILY], weStrings);
});

const placeholderHits = [];
weStrings.forEach(function (s) {
...
const latam = {
  theme: Narrative.HUMAN_THEME_PATTERNS_BY_REGION.LATAM,
  frames: Narrative.SUMMARY_FRAME_POOL_BY_REGION.LATAM,
  observe: Narrative.OBSERVE_TAIL_BY_REGION.LATAM,
  pads: Premium.GOAL_PADS_BY_REGION.LATAM,
  favorece: Premium.SPINE_FAVORECE_OPEN_BY_REGION.LATAM,
  topup: Premium.REGIONAL_TOPUP_VARIANTS.LATAM,
  scene: Premium.HUMAN_SCENE_BY_REGION.LATAM,
  entero: Premium.OBSERVE_ENTERO_TAIL_BY_REGION.LATAM,
  micro: Premium.REGIONAL_EDITORIAL_MICRO_BY_GOAL.LATAM,
  regionalPads: Premium.REGIONAL_EDITORIAL_PADS.LATAM
};
...
assert('rotation HUMAN_THEME ≥3/goal (LATAM parity)', countGoal(we.theme, 3) && countGoal(latam.theme, 3),
  JSON.stringify({ we: we.theme.amor.length, latam: latam.theme.amor.length }));
assert('rotation SUMMARY_FRAME ≥2/goal', countGoal(we.frames, 2) && countGoal(latam.frames, 2), 'ok');
assert('rotation OBSERVE_TAIL ≥2/goal', countGoal(we.observe, 2) && countGoal(latam.observe, 2), 'ok');
assert('rotation GOAL_PADS ≥8/goal', countGoal(we.pads, 8) && countGoal(latam.pads, 8), 'ok');
assert('rotation SPINE_FAVORECE ≥6', we.favorece.length >= 6 && latam.favorece.length >= 6,
  'we=' + we.favorece.length + ' latam=' + latam.favorece.length);
assert('rotation TOPUP ≥10', we.topup.length >= 10 && latam.topup.length >= 10,
  'we=' + we.topup.length + ' latam=' + latam.topup.length);
assert('rotation HUMAN_SCENE ≥3/goal', countGoal(we.scene, 3) && countGoal(latam.scene, 3), 'ok');
assert('rotation OBSERVE_ENTERO ≥2/goal', countGoal(we.entero, 2) && countGoal(latam.entero, 2), 'ok');
assert('rotation MICRO ≥2/goal', countGoal(we.micro, 2) && countGoal(latam.micro, 2), 'ok');
assert('rotation REGIONAL_PADS ≥8', we.regionalPads.length >= 8 && latam.regionalPads.length >= 8,
  'we=' + we.regionalPads.length);
